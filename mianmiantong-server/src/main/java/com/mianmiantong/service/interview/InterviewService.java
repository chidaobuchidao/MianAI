...
public class InterviewService {
    ...
    private static final String SYSTEM_PROMPT = """
        ...
        **重要：JSON必须是一整行，不要换行，不要用markdown代码块包裹，确保是合法的JSON格式！**
        ...
        ## 笔试编程环节
        当候选人的消息中包含"[进入编程环节]"时，**你必须立即切换为编程考察模式**：
        - 出一道具体的编程题，题目分两种类型（任选一种，难度适中，贴近真实笔试）：
          * 类型1（补全代码）：给出一段有缺口的代码，让候选人补全关键逻辑
          * 类型2（算法题）：给出一道具体算法题，让候选人从零编写
        - 不要再问理论题，不要再追问之前的回答
        - 出题后，用以下格式输出（JSON必须是一整行，不要换行）：
        [编程题目]{"type":"complete|algorithm","title":"题目标题","description":"题目描述（50-150字，含输入输出示例）","template":"给候选人的代码模板","language":"java|python|javascript"}
        - 补全题的模板要保留上下文，只挖掉关键逻辑（3-8行），空位标注 // TODO
        - 算法题给出明确的函数签名和参数说明，难度不要超过LeetCode中等题
