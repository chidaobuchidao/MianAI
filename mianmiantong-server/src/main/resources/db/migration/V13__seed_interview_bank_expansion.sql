...
SELECT
    ...
    JSON_OBJECT(
        ...
        'cpp', '#include <bits/stdc++.h>\nusing namespace std;\nint main() {\n    string s; cin >> s;\n    unordered_map<char,char> mp = {{')'\\'', '('}, {']'\\'', '['}, {'}'\\'', '{'}};\n    stack<char> st;\n    for (char c : s) {\n        if (mp.count(c)) { if (st.empty() || st.top() != mp[c]) { cout << "false"; return 0; } st.pop(); }\n        else st.push(c);\n    }\n    cout << (st.empty() ? "true" : "false");\n    return 0;\n}',
...
SELECT
    ...
    JSON_OBJECT(
        'java', 'import java.io.*; import java.util.*; public class Solution { public static void main(String[] args) throws Exception { String s = new BufferedReader(new InputStreamReader(System.in)).readLine().trim(); Map<Character,Integer> v = Map.of(''I'',1,''V'',5,''X'',10,''L'',50,''C'',100,''D'',500,''M'',1000); int ans = 0; for (int i = 0; i < s.length(); i++) { int cur = v.get(s.charAt(i)); int next = i + 1 < s.length() ? v.get(s.charAt(i + 1)) : 0; ans += cur < next ? -cur : cur; } System.out.println(ans); } }',
