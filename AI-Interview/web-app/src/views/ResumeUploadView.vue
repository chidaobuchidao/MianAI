    ...
    <div class="page__inner">
      ...
      <div class="model-bar">
        ...
        <div v-if="hasOptions" class="capsule-toggle">
          <div class="capsule-slider" :style="{ width: (100 / options.length) + '%', transform: 'translateX(' + (options.findIndex(o => o.id === resumeModel) * 100) + '%)' }" />
          <button v-for="opt in options" :key="opt.id" class="capsule-opt" :class="{ active: resumeModel === opt.id }" @click="selectModel(opt.id)">{{ opt.label }}</button>
        </div>
