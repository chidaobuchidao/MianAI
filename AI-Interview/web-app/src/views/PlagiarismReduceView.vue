<template>
  <div class="polish-page">
    ...
    <section class="hero">
      ...
      <span class="hero-label">降查重</span>
      <h1 class="hero-title">降查重</h1>
      <p class="hero-sub">预检重复表达风险，逐段优化降低查重率</p>
    </section>
    ...
    <div v-if="scanResult" class="risk-card">
      ...
      <div v-if="scanResult.features && scanResult.features.length" class="risk-section">
        <div class="risk-section-label">重复短语</div>
        <ul class="risk-issues">
          <li v-for="(f, i) in scanResult.features" :key="i">{{ f }}</li>
        </ul>
      </div>
      <div v-if="scanResult.matchedFragments && scanResult.matchedFragments.length" class="risk-section">
        <div class="risk-section-label">匹配片段</div>
        <ul class="risk-issues">
          <li v-for="(m, i) in scanResult.matchedFragments" :key="i">{{ m }}</li>
        </ul>
      </div>
    </div>
    ...
    <section class="editor-area">
      <div class="panel source">
        ...
        <textarea v-model="sourceText" placeholder="粘贴待降重的论文正文..." class="editor-textarea"></textarea>
        <span class="char-count">{{ sourceText.length }} 字</span>
      </div>
      <div class="panel source">
        <div class="panel-header"><span>重复源</span></div>
        <textarea v-model="sourceRef" placeholder="粘贴被标红的重复原文或参考来源..." class="editor-textarea"></textarea>
        <span class="char-count">{{ sourceRef.length }} 字</span>
      </div>
      <div class="panel result">
        ...
        <div class="result-body" :class="{ placeholder: !output && !isStreaming }">
          ...
          <template v-else>点击「扫描查重」先检测 → 选择降重力度 → 点击「开始降重」</template>
        </div>
      </div>
    </section>
    ...
    <footer class="control-bar">
      ...
      <span class="sep">|</span>
      <span class="mode-label">降重力度:</span>
      <select v-model="mode">
        <option value="light">轻度降重</option>
        <option value="medium">中度降重</option>
        <option value="deep">深度降重</option>
      </select>
      <span class="spacer"></span>
      <button class="btn-primary" @click="startReduce" :disabled="isStreaming || !sourceText.trim()">
        {{ isStreaming ? '降重中...' : '开始降重' }}
      </button>
    </footer>
...
const scanResult = ref<{ score: number; riskLevel: string; features: string[]; matchedFragments: string[] } | null>(null)
...
const riskClass = computed(() =>
  scanResult.value?.riskLevel === '高重复率' ? 'high' :
  scanResult.value?.riskLevel === '中重复率' ? 'medium' : 'low'
)
...
async function scanTraces() {
  ...
  try {
    const token = localStorage.getItem('token') || ''
    const res = await fetch('/api/plagiarism-reduce/scan', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
      body: JSON.stringify({ text: sourceText.value, sourceText: sourceRef.value }),
    })
    const data = await res.json()
...
async function startReduce() {
  await startStream('/api/plagiarism-reduce/run', { text: sourceText.value, sourceText: sourceRef.value, mode: mode.value })
}
</script>
...
.panel.source { flex: 4; border-right: 1px solid rgba(0,0,0,.06); position: relative; }
.panel.result { flex: 5; }
...
.risk-section-label { font-size: 12px; font-weight: 600; color: #4A4A4A; margin-bottom: 6px; }
